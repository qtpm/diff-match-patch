...
void TestDiffMatchPatch::testDiffCommonPrefix()
{
...
void TestDiffMatchPatch::testDiffCommonSuffix()
{
...
void TestDiffMatchPatch::testDiffCommonOverlap()
{
    ...
    assertEquals(
        ...
        dmp.diff_commonOverlap("fi", QString::fromWCharArray((const wchar_t *)L"\ufb01i", 2)));
}

void TestDiffMatchPatch::testDiffHalfmatch()
{
    // Detect a halfmatch.
    dmp.Diff_Timeout = 1;
    ...
    assertEquals("diff_halfMatch: Single Match #1.", QString("12,90,a,z,345678").split(","),
                 dmp.diff_halfMatch("1234567890", "a345678z"));

    assertEquals("diff_halfMatch: Single Match #2.", QString("a,z,12,90,345678").split(","),
                 dmp.diff_halfMatch("a345678z", "1234567890"));

    assertEquals("diff_halfMatch: Single Match #3.", QString("abc,z,1234,0,56789").split(","),
                 dmp.diff_halfMatch("abc56789z", "1234567890"));

    assertEquals("diff_halfMatch: Single Match #4.", QString("a,xyz,1,7890,23456").split(","),
                 dmp.diff_halfMatch("a23456xyz", "1234567890"));

    assertEquals("diff_halfMatch: Multiple Matches #1.",
                 QString("12123,123121,a,z,1234123451234").split(","),
                 dmp.diff_halfMatch("121231234123451234123121", "a1234123451234z"));

    assertEquals("diff_halfMatch: Multiple Matches #2.",
                 QString(",-=-=-=-=-=,x,,x-=-=-=-=-=-=-=").split(","),
                 dmp.diff_halfMatch("x-=-=-=-=-=-=-=-=-=-=-=-=", "xx-=-=-=-=-=-=-="));

    assertEquals("diff_halfMatch: Multiple Matches #3.",
                 QString("-=-=-=-=-=,,,y,-=-=-=-=-=-=-=y").split(","),
...
void TestDiffMatchPatch::testDiffLinesToChars()
{
    ...
    tmpVarList << QVariant::fromValue(QString() + QChar((ushort)1) + QChar((ushort)2)
                                      + QChar((ushort)1)); //(("\u0001\u0002\u0001"));
    tmpVarList << QVariant::fromValue(QString() + QChar((ushort)2) + QChar((ushort)1)
                                      + QChar((ushort)2)); // (("\u0002\u0001\u0002"));
...
void TestDiffMatchPatch::testDiffCharsToLines()
{
...
void TestDiffMatchPatch::testDiffCleanupMerge()
{
    ...
    assertEquals("diff_cleanupMerge: No change case.",
                 diffList(Diff(EQUAL, "a"), Diff(DELETE, "b"), Diff(INSERT, "c")), diffs);
    ...
    diffs = diffList(Diff(DELETE, "a"), Diff(INSERT, "b"), Diff(DELETE, "c"), Diff(INSERT, "d"),
                     Diff(EQUAL, "e"), Diff(EQUAL, "f"));
    dmp.diff_cleanupMerge(diffs);
    assertEquals("diff_cleanupMerge: Merge interweave.",
                 diffList(Diff(DELETE, "ac"), Diff(INSERT, "bd"), Diff(EQUAL, "ef")), diffs);
    ...
    diffs = diffList(Diff(EQUAL, "x"), Diff(DELETE, "a"), Diff(INSERT, "abc"), Diff(DELETE, "dc"),
                     Diff(EQUAL, "y"));
    dmp.diff_cleanupMerge(diffs);
    assertEquals(
    ...
    diffs = diffList(Diff(EQUAL, "a"), Diff(DELETE, "b"), Diff(EQUAL, "c"), Diff(DELETE, "ac"),
                     Diff(EQUAL, "x"));
    dmp.diff_cleanupMerge(diffs);
    assertEquals("diff_cleanupMerge: Slide edit left recursive.",
                 diffList(Diff(DELETE, "abc"), Diff(EQUAL, "acx")), diffs);

    diffs = diffList(Diff(EQUAL, "x"), Diff(DELETE, "ca"), Diff(EQUAL, "c"), Diff(DELETE, "b"),
                     Diff(EQUAL, "a"));
    dmp.diff_cleanupMerge(diffs);
...
void TestDiffMatchPatch::testDiffCleanupSemanticLossless()
{
...
void TestDiffMatchPatch::testDiffCleanupSemantic()
{
    ...
    diffs = diffList(Diff(DELETE, "abc"), Diff(INSERT, "ABC"), Diff(EQUAL, "1234"),
                     Diff(DELETE, "wxyz"));
    dmp.diff_cleanupSemantic(diffs);
    assertEquals("diff_cleanupSemantic: No elimination #2.",
                 diffList(Diff(DELETE, "abc"), Diff(INSERT, "ABC"), Diff(EQUAL, "1234"),
                          Diff(DELETE, "wxyz")),
                 diffs);
    ...
    diffs = diffList(Diff(DELETE, "ab"), Diff(EQUAL, "cd"), Diff(DELETE, "e"), Diff(EQUAL, "f"),
                     Diff(INSERT, "g"));
    dmp.diff_cleanupSemantic(diffs);
    assertEquals("diff_cleanupSemantic: Backpass elimination.",
                 diffList(Diff(DELETE, "abcdef"), Diff(INSERT, "cdfg")), diffs);

    diffs = diffList(Diff(INSERT, "1"), Diff(EQUAL, "A"), Diff(DELETE, "B"), Diff(INSERT, "2"),
                     Diff(EQUAL, "_"), Diff(INSERT, "1"), Diff(EQUAL, "A"), Diff(DELETE, "B"),
                     Diff(INSERT, "2"));
    dmp.diff_cleanupSemantic(diffs);
    ...
    assertEquals("diff_cleanupSemantic: Overlap elimination.",
                 diffList(Diff(DELETE, "abc"), Diff(EQUAL, "xxx"), Diff(INSERT, "def")), diffs);

    diffs = diffList(Diff(DELETE, "xxxabc"), Diff(INSERT, "defxxx"));
    dmp.diff_cleanupSemantic(diffs);
    assertEquals("diff_cleanupSemantic: Reverse overlap elimination.",
                 diffList(Diff(INSERT, "def"), Diff(EQUAL, "xxx"), Diff(DELETE, "abc")), diffs);

    diffs = diffList(Diff(DELETE, "abcd1212"), Diff(INSERT, "1212efghi"), Diff(EQUAL, "----"),
                     Diff(DELETE, "A3"), Diff(INSERT, "3BC"));
    dmp.diff_cleanupSemantic(diffs);
...
void TestDiffMatchPatch::testDiffCleanupEfficiency()
{
    ...
    diffs = diffList(Diff(DELETE, "ab"), Diff(INSERT, "12"), Diff(EQUAL, "xyz"), Diff(DELETE, "cd"),
                     Diff(INSERT, "34"));
    dmp.diff_cleanupEfficiency(diffs);
    assertEquals("diff_cleanupEfficiency: Four-edit elimination.",
                 diffList(Diff(DELETE, "abxyzcd"), Diff(INSERT, "12xyz34")), diffs);
    ...
    diffs = diffList(Diff(DELETE, "ab"), Diff(INSERT, "12"), Diff(EQUAL, "xy"), Diff(INSERT, "34"),
                     Diff(EQUAL, "z"), Diff(DELETE, "cd"), Diff(INSERT, "56"));
    dmp.diff_cleanupEfficiency(diffs);
    assertEquals("diff_cleanupEfficiency: Backpass elimination.",
                 diffList(Diff(DELETE, "abxyzcd"), Diff(INSERT, "12xy34z56")), diffs);
    ...
    assertEquals("diff_cleanupEfficiency: High cost elimination.",
                 diffList(Diff(DELETE, "abwxyzcd"), Diff(INSERT, "12wxyz34")), diffs);
    dmp.Diff_EditCost = 4;
}

void TestDiffMatchPatch::testDiffPrettyHtml()
{
    // Pretty print.
    QList<Diff> diffs = diffList(Diff(EQUAL, "a\n"), Diff(DELETE, "<B>b</B>"), Diff(INSERT, "c&d"));
    assertEquals("diff_prettyHtml:", "<span>a&para;<br></span><del "
                                     "style=\"background:#ffe6e6;\">&lt;B&gt;b&lt;/B&gt;</del><ins "
                                     "style=\"background:#e6ffe6;\">c&amp;d</ins>",
                 dmp.diff_prettyHtml(diffs));
}

void TestDiffMatchPatch::testDiffText()
{
    // Compute the source and destination texts.
    QList<Diff> diffs = diffList(Diff(EQUAL, "jump"), Diff(DELETE, "s"), Diff(INSERT, "ed"),
...
void TestDiffMatchPatch::testDiffDelta()
{
    ...
    */
    ...
    diffs
        = diffList(Diff(EQUAL, QString::fromWCharArray((const wchar_t *)L"\u0680 \000 \t %", 7)),
                   Diff(DELETE, QString::fromWCharArray((const wchar_t *)L"\u0681 \001 \n ^", 7)),
                   Diff(INSERT, QString::fromWCharArray((const wchar_t *)L"\u0682 \002 \\ |", 7)));
    text1 = dmp.diff_text1(diffs);
    assertEquals("diff_text1: Unicode text.",
                 QString::fromWCharArray((const wchar_t *)L"\u0680 \000 \t %\u0681 \001 \n ^", 14),
                 text1);
...
void TestDiffMatchPatch::testDiffXIndex()
{
...
void TestDiffMatchPatch::testDiffLevenshtein()
{
...
void TestDiffMatchPatch::testDiffBisect()
{
    // Normal.
    QString a = "cat";
    QString b = "map";
    ...
    QList<Diff> diffs = diffList(Diff(DELETE, "c"), Diff(INSERT, "m"), Diff(EQUAL, "a"),
...
void TestDiffMatchPatch::testDiffMain()
{
    ...
    diffs = diffList(Diff(EQUAL, "a"), Diff(INSERT, "123"), Diff(EQUAL, "b"), Diff(INSERT, "456"),
                     Diff(EQUAL, "c"));
    assertEquals("diff_main: Two insertions.", diffs, dmp.diff_main("abc", "a123b456c", false));

    diffs = diffList(Diff(EQUAL, "a"), Diff(DELETE, "123"), Diff(EQUAL, "b"), Diff(DELETE, "456"),
                     Diff(EQUAL, "c"));
    ...
    diffs = diffList(Diff(DELETE, "Apple"), Diff(INSERT, "Banana"), Diff(EQUAL, "s are a"),
                     Diff(INSERT, "lso"), Diff(EQUAL, " fruit."));
    assertEquals("diff_main: Simple case #2.", diffs,
                 dmp.diff_main("Apples are a fruit.", "Bananas are also fruit.", false));

    diffs = diffList(Diff(DELETE, "a"),
    ...
    assertEquals(
        ...
        dmp.diff_main("ax\t", QString::fromWCharArray((const wchar_t *)L"\u0680x\000", 3), false));

    diffs = diffList(Diff(DELETE, "1"), Diff(EQUAL, "a"), Diff(DELETE, "y"), Diff(EQUAL, "b"),
                     Diff(DELETE, "2"), Diff(INSERT, "xab"));
    ...
    diffs = diffList(Diff(DELETE, "ABCD"), Diff(EQUAL, "a"), Diff(DELETE, "="), Diff(INSERT, "-"),
                     Diff(EQUAL, "bcd"), Diff(DELETE, "="), Diff(INSERT, "-"),
                     Diff(EQUAL, "efghijklmnopqrs"), Diff(DELETE, "EFGHIJKLMNOefg"));
    assertEquals(
        "diff_main: Overlap #3.", diffs,
        dmp.diff_main("ABCDa=bcd=efghijklmnopqrsEFGHIJKLMNOefg", "a-bcd-efghijklmnopqrs", false));

    diffs = diffList(Diff(INSERT, " "), Diff(EQUAL, "a"), Diff(INSERT, "nd"),
    ...
    b = "abcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghij"
...
void TestDiffMatchPatch::testMatchAlphabet()
{
...
void TestDiffMatchPatch::testMatchBitap()
{
    // Bitap algorithm.
...
void TestDiffMatchPatch::testMatchMain()
{
    // Full match.
...
void TestDiffMatchPatch::testPatchObj()
{
    // Patch Object.
    Patch p;
    p.start1 = 20;
...
void TestDiffMatchPatch::testPatchFromText()
{
...
void TestDiffMatchPatch::testPatchToText()
{
    QString strp = "@@ -21,18 +22,17 @@\n jump\n-s\n+ed\n  over \n-the\n+a\n  laz\n";
    QList<Patch> patches;
...
void TestDiffMatchPatch::testPatchAddContext()
{
    dmp.Patch_Margin = 4;
    Patch p;
...
void TestDiffMatchPatch::testPatchMake()
{
    QList<Patch> patches;
    ...
    assertEquals("patch_make: Text1+Text2+Diff inputs (deprecated)", expectedPatch,
    ...
    diffs
        = diffList(Diff(DELETE, "`1234567890-=[]\\;',./"), Diff(INSERT, "~!@#$%^&*()_+{}|:\"<>?"));
...
void TestDiffMatchPatch::testPatchSplitMax()
{
...
void TestDiffMatchPatch::testPatchAddPadding()
{
    QList<Patch> patches;
...
void TestDiffMatchPatch::testPatchApply()
{
    ...
    assertEquals("patch_apply: Exact match.",
                 "That quick brown fox jumped over a lazy dog.\ttrue\ttrue", resultStr);
    ...
    assertEquals("patch_apply: Partial match.",
                 "That quick red rabbit jumped over a tired tiger.\ttrue\ttrue", resultStr);
    ...
    assertEquals("patch_apply: Failed match.",
                 "I am the very model of a modern major general.\tfalse\tfalse", resultStr);
    ...
    resultStr = results.first + "\t" + (boolArray[0] ? "true" : "false") + "\t"
                + (boolArray[1] ? "true" : "false");
    assertEquals("patch_apply: Big delete, large change 1.", "xabc12345678901234567890-------------"
                                                             "--++++++++++---------------"
...
void TestDiffMatchPatch::assertEquals(const char *strCase, int n1, int n2)
{
    QVERIFY2(n1 == n2, strCase);
}

void TestDiffMatchPatch::assertEquals(const char *strCase, const QString &s1, const QString &s2)
{
    QVERIFY2(s1 == s2, strCase);
}

void TestDiffMatchPatch::assertEquals(const char *strCase, const Diff &d1, const Diff &d2)
{
    QVERIFY2(d1.toString() == d2.toString(), strCase);
}

void TestDiffMatchPatch::assertEquals(const char *strCase, const QList<Diff> &list1,
                                      const QList<Diff> &list2)
{
    QVERIFY2(list1.count() == list2.count(), strCase);
    int i = 0;
...
void TestDiffMatchPatch::assertEquals(const char *strCase, const QList<QVariant> &list1,
                                      const QList<QVariant> &list2)
{
    ...
    foreach (QVariant q1, list1) {
        ...
        QVERIFY2(q1 == q2, strCase);
        i++;
    }
}

void TestDiffMatchPatch::assertEquals(const char *strCase, const QVariant &var1,
                                      const QVariant &var2)
{
    QVERIFY2(var1 == var2, strCase);
}

void TestDiffMatchPatch::assertEquals(const char *strCase, const QMap<QChar, int> &m1,
                                      const QMap<QChar, int> &m2)
{
    ...
    while (i1.hasNext() && i2.hasNext()) {
        i1.next();
        i2.next();
        QVERIFY2(i1.key() == i2.key() && i1.value() == i2.value(), strCase);
    }
...
void TestDiffMatchPatch::assertEquals(const char *strCase, const QStringList &list1,
...
void TestDiffMatchPatch::assertEmpty(const char *strCase, const QStringList &list)
{
    QVERIFY2(list.isEmpty(), strCase);
}

// Private function for quickly building lists of diffs.
QList<Diff> TestDiffMatchPatch::diffList(Diff d1, Diff d2, Diff d3, Diff d4, Diff d5, Diff d6,
                                         Diff d7, Diff d8, Diff d9, Diff d10)
{
...
QTEST_APPLESS_MAIN(TestDiffMatchPatch)
#include "DiffMatchPatch_test.moc"
